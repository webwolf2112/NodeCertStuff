...
// stream that you can call .pipe() on. Here's a readable stream that pipes
// its data to process.stderr:

//    const fs = require('fs')
//    fs.createReadStream('data.txt').pipe(process.stderr)

// Your program is basically the same idea, but instead of 'data.txt', the
// filename comes from process.argv[2] and you should pipe to stdout, not
// stderr.
